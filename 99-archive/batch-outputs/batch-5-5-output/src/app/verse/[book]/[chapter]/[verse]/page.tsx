...
/**
 * Canonical verse page template — redesigned in Batch 5.5.
 *
 * Inline above the fold, in this exact order:
 *   1. Verse text + KJV chip (VerseHeader)
 *   2. Commentary featured excerpt (CommentaryVoices — Doctrine A render)
 *   3. Cross-references top 5 + disclosure (CrossReferenceSection)
 *
 * Below the fold (collapsed):
 *   - Parallel translations (rendered from VerseHeader, defaultOpen=false)
 *   - About this book (rendered from VerseHeader, defaultOpen=false)
 *   - Original language (defaultOpen=false)
 *   - Characters involved / Location / Typological / Prophetic / Manuscript /
 *     Connected graph — all stream via <Suspense>, each returns null when
 *     it has no data.
 *
 * Action bar: Bookmark + Add note + Highlight visible; Compare / Share /
 * Graph-view in the overflow menu. Duplicate SaveVerseButton removed — the
 * account-scoped BookmarkButton covers the "come back to this" surface.
 *
 * "Your notes" textarea now behind a `<details>` affordance at the bottom
 * so it does not eat above-the-fold space before the student has seen
 * content (Batch 5.5 §4 decision).
 *
 * The old big "Explore in graph view" CTA has been demoted into the action
 * bar's overflow menu as a muted "View in graph (beta)" link per Vision v2
 * §5.1 (graph demoted; exploratory feature, not a pillar).
 *
 * Section-numbered eyebrows (01A, 01B, 02, 03, …) are removed from the
 * verse route per Batch 5.5 §4. Plain titles only. The eyebrow prop on
 * `<Section />` is still accepted for call-site compatibility but no longer
 * renders — see Section.tsx.
 */
    ...
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      <main className="pt-6 pb-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          {/* ─── 1. Verse hero — verse text + translation chips + collapsed
                 parallel-translations + collapsed about-book */}
          <VerseHeader
            bookId={bookData.id}
            bookName={bookData.name}
            bookAbbr={lowerAbbr}
          ...
          {/* ─── Action bar (compact, deduplicated) */}
          ...
          {/* ─── 2. Commentary featured excerpt — Doctrine A, Batch 4+5 shipped.
                 Preserved inline above the fold per Batch 5.5 §4. */}
          <Suspense fallback={<SectionSkeleton title="Commentary voices" />}>
            <CommentaryVoices bookName={bookData.name} chapterNum={chapterNum} />
          </Suspense>

          {/* ─── 3. Cross-references top 5 + "Show all N" disclosure. */}
          <Suspense fallback={<SectionSkeleton title="Cross-references" />}>
            <CrossReferenceSection graphNodeId={graphNodeId} />
          </Suspense>

          {/* ─── Below the fold: collapsed sections. Each returns null when
                 it has no data, so empty headers don't accumulate. */}
          <OriginalLanguage kjvText={kjvText} testament={bookData.testament} />
          ...
          {/* ─── "Your notes" — collapsed affordance, not above-the-fold. */}
          ...
          {/* Footer CTA — single "Continue reading" button.
              The old gold "Explore in graph view" CTA is gone; graph access
              moved into the action-bar overflow menu per Vision v2 §5.1. */}
